...
BeCameraStatusCode BeCameraInit(BeCameraHandle *becameraHandleAddress)
{
	...
	*becameraHandleAddress = (BeCameraHandle)handle;
	...
	// 初始化打开相机时的参数
	// 配置采集格式
	av_dict_set(&handle->openCameraOpts, "input_format", "mjpeg", 0);
	// 配置帧率
	av_dict_set(&handle->openCameraOpts, "framerate", "30", 0);
	// 配置分辨率
	av_dict_set(&handle->openCameraOpts, "video_size", "1920*1080", 0);
...
BeCameraStatusCode BeCameraOpenDevice(BeCameraHandle becameraHandle, uint8_t cameraID)
{
	...
	// 打开指定的视频输入设备
	int ret = avformat_open_input(&handle->avFmtCtx, deviceName.c_str(), handle->avInFmt, &handle->openCameraOpts);
	if (ret != 0)
	{
		return BeCameraStatusCode::ERR_OPEN_VIDEO_INPUT_DEVICE;
	}
	...
	if (ret < 0)
	{
		// 关闭已打开的视频设备
		avformat_close_input(&handle->avFmtCtx);
		return BeCameraStatusCode::ERR_FIND_DEVICE_STREAM_INFO;
	}
	...
	if (inVideoStreamIndex == -1)
	{
		// 关闭已打开的视频设备
		avformat_close_input(&handle->avFmtCtx);
		return BeCameraStatusCode::ERR_NOT_SUPPORT_VIDEO_STREAM;
	}
